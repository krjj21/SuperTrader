...
# A/B 비교 sweeps (모두 같은 train+pool 사이클 안에서 OFF/ON 실행 — fair compare)
python scripts/atr_filter_compare.py             # E1 ATR 진입 필터 OFF/ON
python scripts/confidence_sizing_compare.py      # confidence_sizing OFF/ON_down/ON_up
python scripts/buy_threshold_sweep.py            # buy_action_threshold 그리드
python scripts/investor_filter_compare.py        # B1 foreign vs foreign+organ
python scripts/cap_rank_top_n_sweep.py           # cap_rank × top_n 그리드 (2026-05-06 신규)
python scripts/partial_tp_compare.py             # C+E 부분익절+트레일링+LLM RSI 보류 (2026-05-07 신규)

# 백테스트 기간 regime 라벨 backfill (1회 또는 backtest.start/end 변경 후)
python scripts/backfill_regime_labels.py                     # backtest start~end 전체
python scripts/backfill_regime_labels.py --start 20180101 --end 20241231 --force
...
- **FactorHybridStrategy**: alpha model (XGBoost or Transformer) + RL (timing/risk). BUY requires both; SELL on alpha alone (with confidence gate). Constructor accepts `ml_model_type ∈ {xgboost, transformer}` and explicit `ml_buy_threshold`/`ml_sell_threshold` overrides; otherwise falls back to `xgb_*_confidence_threshold` (0.55/0.60) for XGB or `transformer_*_confidence_threshold` (0.70/0.75) for transformer (softmax flatter → higher gate needed). Defaults to XGB.
  - Past peak: 2026-04-20 sweep on legacy universe → **+63.3% / Sharpe 0.45 / MDD −21.8% @ buy_action_threshold=0.07**. *주의: 이 수치도 그 시점 모델 + legacy universe 의존 fragile baseline. 현재 코드/PIT universe 에서 재현 안 될 수 있음 — 인용 전 fresh 재측정.*
  - **현재 baseline (2026-05-07 fresh, fd=5, top_n=30, biweekly, OOS 92회)**: factor_hybrid **+3.9% / Sharpe −1.23 / MDD −6.17% / 145 trades / profit_factor 1.12**. *과거 메모의 "+638%" / "+515%" 같은 거대 수치는 단위 표기 오류였음* — `backtest/metrics.py:34` 가 `total_return = (final/init - 1) × 100` 으로 *이미 백분율* 을 raw 값으로 출력하는데, 누군가 raw `5.15` / `6.38` 을 *비율* 로 오인하고 다시 ×100 해서 "+515%/+638%" 로 옮긴 것. 실제 그 시점 측정은 +5.15% / +6.38% 였음. 진짜 변동 폭은 5-06 sweep top_n=30 = +14.55% → 5-07 fresh = +3.9% 로 *모델 학습 사이클* 차이 −10.6%p (sweep 은 기존 모델 로드, fresh 는 재학습). 베이스라인 인용 전 항상 동일 사이클에서 재측정 + raw 단위 (이미 백분율) 확인. 리포트: `logs/{train,backtest}_fd5_20260506_220251.log`. Transformer variants (factor_hybrid_transformer, tx_p80, tx_p85) all **net-negative with 700-880 trades** — overtrading, do NOT promote to production.
...
- **Backtest engine** (`portfolio_engine.py`): `old_set - new_set` force-SELL always runs (not gated on `commit_pool`). 2026-05-07: force pool_exit 의 pending_orders 를 `is_stop=True` 로 표시 — 갭 패널티 적용 + C 의 partial sell 분기 자동 제외 (전량 매도 보장). Only new-entry auto-BUY is restricted to `commit_pool` strategies (FactorOnly).
...
### Pool Pre-filters (Stage 1 + 2 add-ons)

Optional pre-filters applied **before** the 220-factor IC scoring (Stage 1 = universe, Stage 2 = factor):

- **Cap rank filter** (`universe.cap_rank_min`, `universe.cap_rank_max`): rank-based cap selection. e.g. `30/150` = mid-cap rank 30-150th. Activates only when both > 0. **2026-05-06 grid sweep 에서 *무의미한 변수*로 확인**: 5개 다른 cap_rank 페어 (0,0)/(1,30)/(1,100)/(30,100)/(30,150) 가 모두 *동일 풀* 생성 (IC-weighted composite + neutralizer 가 cap_rank 필터와 독립적으로 같은 종목 선택). 이 결론은 fresh train+backtest 와 무관한 *결정론적* 사실 (풀 빌드 자체가 deterministic). *그 sweep 시 기록된 절대 수치 (top_n=15→+6.38%, top_n=30→+14.55%) 는 그 시점 모델 의존* — 5-07 fresh 비교에서 fd=5/top_n=30 = +3.9% 로 측정. 단 거대 수치 "+515%" 식의 표기는 단위 혼동이었음 (raw 백분율을 ×100 함). 현재 `0/0` 비활성 유지. Implemented in `_legacy_get_universe()` and `get_universe()` (PIT); rank is computed on **today's snapshot cap** (Tier 3 PIT limit — backtest "rank 30-150" is *current mid-cap's historical behavior*, not point-in-time).
...
- Together with `factors.top_n` (currently **30**; 2026-05-06 sweep 에서 top_n=30 이 top_n=15 보다 우월하다는 *방향성* 확인 — 다만 절대 수치는 모델 의존이라 인용 금지) and `rebalance_freq` (monthly/biweekly), this forms a 4-stage funnel: cap rank → foreign net-buy top % → 220-factor IC → top N.

`pool_cache._payload()` includes `cap_rank_min/max` in the cache key (added in 0358742) — toggling cap_rank invalidates pool caches automatically. Re-enabling mid-cap mode requires fresh backtest evidence; the live pool will rebuild on the next monthly rebalance, so **restart `main.py live` after a settings flip** to avoid stale in-memory pool causing forced exits.
...
- **risk/manager.py**: 5%/position, -5% daily loss limit, -7% stop loss, kill switch (3 consecutive errors). `is_trading_allowed` blocks weekends + Korean holidays via `src/utils/market_calendar.py`. `calculate_position_size(regime_label=...)` applies a multiplier when `regime.enabled and regime.lambda_>0`.
  - **Confidence-proportional sizing (2026-05-01 도입)**: `confidence_sizing_enabled=true, mode=scale, min=1.0, max=2.0`. Hybrid `TradeSignal.strength` 가 XGB `predict_proba_last` 기반 0.5~1.0 으로 들어오면 `min + (max-min) × ((strength-0.5)/0.5)` 선형 매핑으로 슬롯 배수가 결정됨 (1.0× ~ 2.0×). `mode=clamp` 도 구현되어 있어 하향 사이징이 필요하면 `min=0.5, max=1.0` 으로 되돌릴 수 있음. 라이브와 백테스트 양쪽에서 동일 공식(`src/risk/manager.py` + `backtest/portfolio_engine.py`). *측정 효과 (4-30 confidence_sizing_compare 사이클 안에서)*: OFF=+5.15%, ON_down=+3.18%, **ON_up=+6.38%** → ON_up 채택. 이전 기록의 "+515% → +638%" 는 raw 백분율 (5.15, 6.38) 을 *비율* 로 오인하고 ×100 한 단위 표기 오류 — 실제 효과는 약 +1.23%p (OFF→ON_up). 같은 sweep 안의 상대 비교라 신뢰 가능.
  - **C: 부분 익절 + 트레일링 스탑 (2026-05-07 구현, 미채택)**: `partial_take_profit_enabled` (default false), `partial_take_profit_pct=0.50`, `trailing_stop_pct=0.03`. Backtest 만 구현 — `BacktestPosition.peak_price/partial_taken` 추적, `_sell(sell_qty=...)` 분기, signal SELL → 1차 50% 매도 → 잔여는 peak 대비 -3% 시 매도. *2026-05-07 OOS 92회 A/B*: OFF=+5.20% / ON=+3.77% → -1.43%p 악화 + MDD -1.49%p 악화 + trades +44%. **default 파라미터 미채택**. 코드는 토글로 보존 — 추후 sweep (partial_pct / trailing_pct / rsi_threshold) 가능. 라이브 미적용.
  - **E: LLM SELL 보류 강화 (2026-05-07, 미채택)**: `llm_strong_momentum_hold_enabled` (default false), RSI ≥ `llm_strong_momentum_rsi_threshold` (default 80) 단독 충족 시 SELL 보류. `src/timing/llm_validator.py:_rule_check_sell` 의 5조건 외 분기. C 와 함께 도입 시 SELL 보류 ↑ + 부분 익절 보호 의도였으나 위 A/B 에서 같이 OFF 보다 열등 — 둘 다 OFF 유지.
