...
def evaluate_rl_agent(
    ...
    Returns:
        sharpe, total_return, max_drawdown 등 성과 지표
    """
    ...
    sharpe = mean_ret / std_ret * np.sqrt(250)  # 연환산
...
def train_rl_model(
    ...
    logger.info(f"GRPO 학습 데이터: {len(train_dict)}종목, val: {len(val_dict)}종목")
    ...
    agent = RLTimingModel(
        ...
        clip_epsilon_low=rl_config.clip_epsilon_low,
        clip_epsilon_high=rl_config.clip_epsilon_high,
    ...
    logger.info(f"GRPO 학습 시작: {n_episodes} 에피소드 × {len(codes)} 종목")
    ...
    for episode in range(n_episodes):
        ...
        for code in codes:
            ...
            try:
                ...
                # GRPO 업데이트
                states = torch.FloatTensor(rollout["states"]).to(agent.device)
                actions = torch.LongTensor(rollout["actions"]).to(agent.device)
                advantages = torch.FloatTensor(rollout["advantages"]).to(agent.device)
                old_log_probs = torch.FloatTensor(rollout["log_probs"]).to(agent.device)

                stats = agent.grpo_update(states, actions, advantages, old_log_probs)
                episode_losses.append(stats["policy_loss"])
        ...
        if (episode + 1) % 5 == 0 or episode == n_episodes - 1:
            ...
            logger.info(
                ...
                f"avg_reward={avg_reward:.4f}, loss={avg_loss:.4f}, "
                f"val_sharpe={val_sharpe:.3f}, val_return={val_metrics['total_return']:.2f}%"
            )
            ...
            if val_sharpe > best_sharpe:
                ...
                logger.info(f"  → 최고 Sharpe 갱신: {best_sharpe:.3f}")
        elif (episode + 1) % 10 == 0:
            logger.info(
                f"Episode {episode+1}/{n_episodes}: "
                f"avg_reward={avg_reward:.4f}, loss={avg_loss:.4f}"
    ...
    logger.info(
        f"GRPO 학습 완료: sharpe={final_metrics['sharpe']:.3f}, "
